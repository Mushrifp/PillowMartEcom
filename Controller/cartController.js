        ...
        for (let i = 0; i < arrayOfID.length; i++) {
          ...
          if (arrayOfID[i].id == productdata._id) {
            ...
            await cart.updateOne(
              { user: req.session.user_id, "product.id": arrayOfID[i].id },
              { $set: { "product.$.total": tot } }
            );
  ...
  try {
    if (req.session.user_id) {
      ...
      if (cartDB) {
        ...
        const done = await cart.findOneAndUpdate(
          { user: req.session.user_id },
          { $addToSet: { product: newCarts } }
        );
...
const addWish = async (req, res) => {
  try {
    if (req.session.user_id) {
      ...
      if (wishDB) {
        const done = await wish.findOneAndUpdate(
          { user: req.session.user_id },
          { $addToSet: { product: req.query.id } }
        );
        done ? res.send({ added: "Added" }) : res.send({ failed: "Not Added" });
      } else {
...
const qtyCart = async (req, res) => {
  try {
    if (req.body.action == "increment") {
      ...
      const matchingItem = userCart.product.find(
      ...
      if (matchingItem.quantity >= 10) {
        ...
        if (productInfo.stock > matchingItem.quantity) {
          await cart.updateOne(
            { user: req.session.user_id, "product.id": req.body.id },
          ...
          let totSub = totArr.product.reduce(
