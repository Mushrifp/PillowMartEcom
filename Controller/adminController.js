...
const loadDash = async (req, res) => {
  try {
    ...
    const revenue = await order.aggregate([
      { $unwind: "$items" },
      { $match: { "items.paymentMethod": { $in: ["wallet", "razorpay"] } } },
      { $group: { _id: null, totalRevenue: { $sum: "$items.cash" } } },
    ]);
  ...
  try {
    ...
    if (req.body.productTitle) {
      ...
      if (existingProduct && existingProduct._id != req.body.id) {
        res.render("edit-product-3", {
          message: "This product name already exists",
          categoryData,
          currentData: productDatas,
        });
      }
      await productData.findByIdAndUpdate(
        { _id: req.body.id },
        { $set: { productTitle: req.body.productTitle } }
      );
    }
    ...
    if (req.files) {
      if (req.files["images0"]) {
        await productData.findByIdAndUpdate(
          { _id: req.body.id },
          { $set: { "image.0": req.files["images0"][0].filename } }
        );
      ...
      if (req.files["images1"]) {
        await productData.findByIdAndUpdate(
          { _id: req.body.id },
          { $set: { "image.1": req.files["images1"][0].filename } }
        );
      ...
      if (req.files["images2"]) {
        await productData.findByIdAndUpdate(
          { _id: req.body.id },
          { $set: { "image.2": req.files["images2"][0].filename } }
        );
    ...
    res.render("edit-product-3", {
      ...
      categoryData,
      currentData: updatedProductData,
    });
  } catch (error) {
    console.log(error);
  }
  ...
  try {
    ...
    if (!existingCategory) {
      ...
      res.render("categories", {
        ...
        pdDescription: req.body.description,
        categoryData,
      });
    }
  } catch (error) {
...
const editCategory = async (req, res) => {
  try {
    ...
    const done = await categoryDB.findByIdAndUpdate(
      { _id: req.body.id },
...
const viewOrder = async (req, res) => {
  try {
    let done = await order.findOne(
      { userID: req.query.user, "items._id": req.query.id },
      { "items.$": 1 }
    );

    const options = { year: "numeric", month: "long", day: "numeric" };
    const ordered = done.items[0].Dates.ordered.toLocaleDateString(
      "en-US",
      options
    );
    const delivery = done.items[0].Dates.delivery.toLocaleDateString(
      "en-US",
      options
    );
    let obj = {
...
const statusChange = async (req, res) => {
  try {
    ...
    let done = await order.updateOne(
      { userID: req.query.userId, "items._id": objectId },
      { $set: { "items.$.status": req.query.action } }
    );
...
const addCoupon = async (req, res) => {
  try {
    const { code, name, description, discount, count, startDate, endDate } =
      req.body;
    ...
    let o = await order.aggregate([
      { $unwind: "$items" },
      {
        $group: {
          ...
          wallet: {
            $sum: {
              $cond: [{ $eq: ["$items.paymentMethod", "wallet"] }, 1, 0],
            },
          },
          razorpay: {
            $sum: {
              $cond: [{ $eq: ["$items.paymentMethod", "Razorpay"] }, 1, 0],
            },
          },
        },
      },
...
const filterSales2 = async (req, res) => {
  try {
    ...
    if (query == "Monthly") {
      ...
      send = dataToSend.filter(
        (item) => item._id.orderedDate === todayFormatted
      );
    }
...
const editOfferSave = async (req, res) => {
  try {
    ...
    const updatedOffer = await offer.findByIdAndUpdate(objectId, updateFields, {
      new: true,
    });
...
const apply = async (req, res) => {
  try {
    ...
    const updatedProduct = await product.updateOne(
      { _id: req.query.id },
      { $set: { offPrice: finalPrice, percentage: offerDetails.discount } }
    );
...
const removeOfferProduct = async (req, res) => {
  try {
    ...
    const productCategory = await product.findOne({
      _id: req.query.productdID,
    });

    const categoryCheck = await categoryDB.findOne({
    ...
    if (categoryCheck.offer != true) {
      await offer.updateOne(
        { _id: req.query.offer },
        { $pull: { offItem: req.query.productdID } }
      );
      await product.updateOne(
        { _id: req.query.productdID },
        { $set: { offPrice: 0, percentage: 0 } }
      );
      ...
      const producDetails = await product.findOne({
        _id: req.query.productdID,
      });
      ...
      await product.updateOne(
        { _id: req.query.productdID },
        { $set: { offPrice: finalPrice, percentage: offerInfos[0].discount } }
      );
      await offer.updateOne(
        { _id: req.query.offer },
        { $pull: { offItem: req.query.productdID } }
...
const currentOfferCategory = async (req, res) => {
  try {
    ...
    if (arrayID.length != 0) {
      ...
      res.render("currentOfferCategory", {
...
const removeOfferCategory = async (req, res) => {
  try {
    ...
    const detailsCategory = await categoryDB.findOne({
    ...
    await offer.updateOne(
      ...
      { $pull: { offItem: req.body.categoryID } }
    );
    await categoryDB.updateOne(
      { _id: req.body.categoryID },
      { $set: { offer: false } }
  ...
  try {
    ...
    for (let i = 0; i < done.length; i++) {
      let data = await order.findOne({
        userID: done[i].userId,
        "items._id": done[i].orderId,
      });
